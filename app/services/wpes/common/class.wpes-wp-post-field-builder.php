...
class WPES_WP_Post_Field_Builder extends WPES_Abstract_Field_Builder {
	...
	function post_fields($post, $lang, $index_mlt_content = false, $index_custom_taxonomy = true, $multi_lang_support = false) {
		...
		$post_title = ['default' => $this->remove_shortcodes($this->clean_string($post->post_title)) ];
		$post_content = ['default' =>  $this->remove_shortcodes($this->clean_string($post->post_content)) ];
		$post_excerpt = ['default' => $this->remove_shortcodes($this->clean_string($post->post_excerpt)) ];
